...
final class JFLAC__Metadata_SimpleIterator extends Jmetadata_iterators {
	...
	public JFLAC__Metadata_SimpleIterator()
	{
		file = null;
		filename = null;
	...
	private final void simple_iterator_free_guts_()
	{
	...
	public final void FLAC__metadata_simple_iterator_delete()
	{
		//FLAC__ASSERT(0 != iterator);

		simple_iterator_free_guts_();
	}
	 ...
	 */
	public final int /* FLAC__Metadata_SimpleIteratorStatus */ FLAC__metadata_simple_iterator_status()
	{
		//FLAC__ASSERT(0 != iterator);
	...
	private final boolean read_metadata_block_header_()
	{
		...
		final Jmetadata_block_header_helper h;
	...
	private final boolean simple_iterator_prime_input_(final boolean read_only)
	{
		//FLAC__ASSERT(0 != iterator);
		try {
			if( read_only ) {
	 ...
	 */
	public final boolean FLAC__metadata_simple_iterator_init(final String file_name, final boolean read_only, final boolean preserve_file_stats)
	{
		...
		return simple_iterator_prime_input_( read_only );
	}
	 ...
	 *    See above.
	 */
	public final boolean FLAC__metadata_simple_iterator_is_writable()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 *    \c true.
	 */
	public final boolean FLAC__metadata_simple_iterator_next()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 *    \c true.
	 */
	public final boolean FLAC__metadata_simple_iterator_prev()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 */
	/*@@@@add to tests*/
	public final boolean FLAC__metadata_simple_iterator_is_last()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 */
	/*@@@@add to tests*/
	public final long FLAC__metadata_simple_iterator_get_block_offset()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 */
	public final int /* FLAC__MetadataType */ FLAC__metadata_simple_iterator_get_block_type()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 */
	/*@@@@add to tests*/
	public final int FLAC__metadata_simple_iterator_get_block_length()
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 */
	/*@@@@add to tests*/
	public final boolean FLAC__metadata_simple_iterator_get_application_id(final byte[] id)
	{
		...
		if( this.type != Jformat.FLAC__METADATA_TYPE_APPLICATION ) {
	...
	private final boolean read_metadata_block_data_(final JFLAC__StreamMetadata block)
	{
		//FLAC__ASSERT(0 != iterator);
		//FLAC__ASSERT(0 != iterator->file);
	 ...
	 *    allocation error.
	 */
	public final JFLAC__StreamMetadata FLAC__metadata_simple_iterator_get_block()
	{
		final JFLAC__StreamMetadata block = JFLAC__StreamMetadata.FLAC__metadata_object_new( this.type );
		...
		if( null != block ) {
			...
			if( ! read_metadata_block_data_( block ) ) {
				JFLAC__StreamMetadata.FLAC__metadata_object_delete( block );
				return null;
			}

			/* back up to the beginning of the block data to stay consistent */
			try {
				this.file.seek( this.offset[this.depth] + Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH );
			} catch(final IOException e) {
				this.status = FLAC__METADATA_SIMPLE_ITERATOR_STATUS_SEEK_ERROR;
				JFLAC__StreamMetadata.FLAC__metadata_object_delete( block );
				return null;
	...
	private final boolean write_metadata_block_stationary_(final JFLAC__StreamMetadata block)
	{
		try {
	...
	private final boolean write_metadata_block_stationary_with_padding_(final JFLAC__StreamMetadata block, final int padding_length, final boolean padding_is_last)
	{
		...
		try {
			...
			if( null == (padding = JFLAC__StreamMetadata.FLAC__metadata_object_new( Jformat.FLAC__METADATA_TYPE_PADDING )) ) {
			...
			if( FLAC__METADATA_SIMPLE_ITERATOR_STATUS_OK != (this.status = write_metadata_block_header_( this.file, padding )) ) {
				JFLAC__StreamMetadata.FLAC__metadata_object_delete( padding );
				return false;
			}
	...
	private final boolean simple_iterator_copy_file_prefix_(RandomAccessFile tempfile, String tempfilename, boolean append)
	{
		final long offset_end = append ? this.offset[this.depth] + (long)Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + (long)this.length : this.offset[this.depth];
		...
		if( JFLAC__Metadata_SimpleIterator.FLAC__METADATA_SIMPLE_ITERATOR_STATUS_OK != (this.status = copy_n_bytes_from_file_( this.file, tempfile, offset_end )) ) {
			cleanup_tempfile_( tempfile, tempfilename );
			return false;
		}
	...
	*/
	...
	private final boolean simple_iterator_copy_file_postfix_(final RandomAccessFile tempfile, final String tempfilename, final int fixup_is_last_code, final long fixup_is_last_flag_offset, final boolean backup)
	{
		final long save_offset = this.offset[this.depth];
		//FLAC__ASSERT(0 != *tempfile);

		try {
			this.file.seek( save_offset + Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + this.length );
		} catch(final IOException e) {
			cleanup_tempfile_( tempfile, tempfilename );
			this.status = FLAC__METADATA_SIMPLE_ITERATOR_STATUS_SEEK_ERROR;
			return false;
		...
		if( ! simple_iterator_prime_input_( ! this.is_writable ) ) {
			return false;
		}
		if( backup ) {
			while( this.offset[this.depth] + Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + this.length < save_offset ) {
			...
			this.depth++;
			return simple_iterator_pop_();
		}
	}

	private final boolean rewrite_whole_file_(final JFLAC__StreamMetadata block, final boolean append)
	{
		...
		if( this.is_last ) {
			if( append ) {
				...
				simple_iterator_push_();
				if( ! FLAC__metadata_simple_iterator_prev() ) {
					simple_iterator_pop_();
					return false;
				}
				fixup_is_last_code = -1; /* -1 => set the is_last the flag at the following offset */
				fixup_is_last_flag_offset = this.offset[this.depth];
				if( ! simple_iterator_pop_() ) {
					return false;
				}
			}
		}
		...
		*/
		...
		final long offset_end = append ? this.offset[this.depth] + Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + this.length : this.offset[this.depth];
		...
		if( ! simple_iterator_copy_file_postfix_( tempfile, tempfilename, fixup_is_last_code, fixup_is_last_flag_offset, block == null ) ) {
			return false;
		}

		if( append ) {
			return FLAC__metadata_simple_iterator_next();
		}

		return true;
	}

	private final void simple_iterator_push_()
	{
	...
	private final boolean simple_iterator_pop_()
	{
		//FLAC__ASSERT(iterator->depth > 0);
		this.depth--;
		try {
	 ...
	 */
	public final boolean FLAC__metadata_simple_iterator_set_block(final JFLAC__StreamMetadata block, boolean use_padding)
	{
		...
		if( this.type == Jformat.FLAC__METADATA_TYPE_STREAMINFO || block.type == Jformat.FLAC__METADATA_TYPE_STREAMINFO ) {
		...
		if( this.length == block.length) {
			...
			if( use_padding && this.length >= Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + block.length ) {
				final boolean ret = write_metadata_block_stationary_with_padding_( block, this.length - Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH - block.length, block.is_last );
			...
			if( use_padding ) {
				...
				if( this.is_last ) {
					...
					if( this.type != Jformat.FLAC__METADATA_TYPE_PADDING ) {
						use_padding = false;
					}
					else {
						if( Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + this.length == extra_padding_bytes_required ) {
							padding_leftover = 0;
							block.is_last = this.is_last;
						}
							...
							padding_leftover = Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + this.length - extra_padding_bytes_required;
			...
			if( use_padding ) {
				if( padding_leftover == 0 ) {
					...
					final boolean ret = write_metadata_block_stationary_with_padding_( block, padding_leftover - Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH, padding_is_last );
					//FLAC__ASSERT(!ret || iterator->offset[iterator->depth] == debug_target_offset);
					//FLAC__ASSERT(!ret || ftello(iterator->file) == debug_target_offset + (off_t)FLAC__STREAM_METADATA_HEADER_LENGTH);
					return ret;
				}
	 ...
	 *    \c true if successful, else \c false.
	 */
	public final boolean FLAC__metadata_simple_iterator_insert_block_after(final JFLAC__StreamMetadata block, boolean use_padding)
	{
		...
		if( use_padding ) {
			...
			if( this.is_last ) {
				...
				if( this.type != Jformat.FLAC__METADATA_TYPE_PADDING ) {
					...
					else if( this.length < Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + block.length ) {
						use_padding = false;
					} else {
	 ...
	 *    \c true if successful, else \c false.
	 */
	public final boolean FLAC__metadata_simple_iterator_delete_block(final boolean use_padding)
	{
		//FLAC__ASSERT_DECLARATION(off_t debug_target_offset = iterator->offset[iterator->depth];)
		...
		if( this.type == Jformat.FLAC__METADATA_TYPE_STREAMINFO ) {
			this.status = FLAC__METADATA_SIMPLE_ITERATOR_STATUS_ILLEGAL_INPUT;
			return false;
		}

		if( use_padding ) {
			final JFLAC__StreamMetadata padding = JFLAC__StreamMetadata.FLAC__metadata_object_new( Jformat.FLAC__METADATA_TYPE_PADDING );
			if( null == padding ) {
				this.status = FLAC__METADATA_SIMPLE_ITERATOR_STATUS_MEMORY_ALLOCATION_ERROR;
				return false;
			}
			padding.length = this.length;
			if( ! FLAC__metadata_simple_iterator_set_block( padding, false ) ) {
				JFLAC__StreamMetadata.FLAC__metadata_object_delete( padding );
				return false;
			}
			JFLAC__StreamMetadata.FLAC__metadata_object_delete( padding );
