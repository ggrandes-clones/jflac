...
*/
	 ...
	 */
	private static void FLAC__MD5Transform(final int buf[], final int in[])
	{
		int a = buf[0];
		int b = buf[1];
		int c = buf[2];
	 ...
	 * of bytes.
	 */
	private final void FLAC__MD5Update(final byte[] buff, int len)
	{
		/* Update byte count */
		...
		FLAC__MD5Transform( this.buf, this.in );
		int buf_offset = t;
		len -= t;
	 ...
	 */
	final void FLAC__MD5Final(final byte digest[])
	{
		...
		if( count < 0 ) {	/* Padding forces an extra block */
			...
			FLAC__MD5Transform( this.buf, input );
			count = 56;
			offset = 0;
		}
	 ...
	 */
	final boolean FLAC__MD5Accumulate(final int signal[][], final int channels, final int samples, final int bytes_per_sample)
	{
		/* overflow check */
		if( channels > Jformat.SIZE_MAX ) {
			return false;
		}
		...
		FLAC__MD5Update( this.internal_buf, bytes_needed );
