 ...
 */
class Jmetadata_iterators {// FIXME why shared functions return FLAC__Metadata_SimpleIterator statuses?
	...
	static final class Jmetadata_block_header_helper {
		boolean is_last;
	...
	static int seek_to_first_metadata_block_cb_(
			...
			//JFLAC__IOHandle handle, JFLAC__IOCallback_Read read_cb, JFLAC__IOCallback_Seek seek_cb)
	{
		...
		if( 0 == Jformat.memcmp( buffer, 0, JFLAC__StreamDecoder.ID3V2_TAG_, 0, 3 ) ) {
			int tag_length = 0;
		...
		if( 0 == Jformat.memcmp( Jformat.FLAC__STREAM_SYNC_STRING, 0, buffer, 0, Jformat.FLAC__STREAM_SYNC_LENGTH ) ) {
			return 0;
		} else {
			return 3;
		}
	...
	/** @return Jmetadata_block_header_helper */// java: non static for correct access to helper class
	static Jmetadata_block_header_helper /* boolean */ read_metadata_block_header_cb_(//JFLAC__IOHandle handle, JFLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle)// java: changed
			//boolean[] is_last, int[] /*FLAC__MetadataType*/ type, int[] length)
	{
		final byte raw_header[] = new byte[Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH];

		try {
			if( handle.read( raw_header, 0, Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH ) != Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH )
			 {
				return null;// false;
			}
		} catch(final Exception e) {
		...
		final Jmetadata_block_header_helper helper = new Jmetadata_block_header_helper();
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_streaminfo_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_StreamInfo block)
	{
		final byte buffer[] = new byte[JFLAC__StreamMetadata_StreamInfo.FLAC__STREAM_METADATA_STREAMINFO_LENGTH];

		try {
			handle.read( buffer, 0, JFLAC__StreamMetadata_StreamInfo.FLAC__STREAM_METADATA_STREAMINFO_LENGTH );
		} catch(final IOException e) {
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_padding_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Seek seek_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_Padding block, final int block_length)
	{
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_application_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_Application block, int block_length)
	{
		final int id_bytes = Jformat.FLAC__STREAM_METADATA_APPLICATION_ID_LEN / 8;
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_seektable_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_SeekTable block, final int block_length)
	{
		final byte buffer[] = new byte[Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH];

		//FLAC__ASSERT(block_length % FLAC__STREAM_METADATA_SEEKPOINT_LENGTH == 0);

		block.num_points = block_length / Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH;

		if( block.num_points == 0 ) {
			block.points = null;
		} else if( null == (block.points = new JFLAC__StreamMetadata_SeekPoint[block.num_points]) ) {
			return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_MEMORY_ALLOCATION_ERROR;
		}

		try {
			for( int i = 0; i < block.num_points; i++ ) {
				final JFLAC__StreamMetadata_SeekPoint p = new JFLAC__StreamMetadata_SeekPoint();
				block.points[i] = p;
				if( handle.read( buffer, 0, Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH ) != Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH ) {
					return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
				}
	...
	{
		final int entry_length_len = Jformat.FLAC__STREAM_METADATA_VORBIS_COMMENT_ENTRY_LENGTH_LEN / 8;
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_vorbis_comment_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb, FLAC__IOCallback_Seek seek_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_VorbisComment block,
			int block_length)
	{
		//FLAC__Metadata_SimpleIteratorStatus status;
		final int num_comments_len = Jformat.FLAC__STREAM_METADATA_VORBIS_COMMENT_NUM_COMMENTS_LEN / 8;
		final byte buffer[] = new byte[4]; /* magic number is asserted below */
		byte[] data;
		...
		try{
			try {
				...
				block.vendor_string = new String( data, JFLAC__StreamMetadata_VorbisComment.ENCODING );
			...
			for( int i = 0; i < block.num_comments; i++ ) {
				try{
					...
					block.comments[i] = new String( data, JFLAC__StreamMetadata_VorbisComment.ENCODING );
...
//skip:
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_cuesheet_track_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_CueSheet_Track track)
	{
		...
		try {
			int len = Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_OFFSET_LEN / 8;
			...
			len = Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_NUMBER_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_ISRC_LEN / 8;
			...
			len = (Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_TYPE_LEN + Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_PRE_EMPHASIS_LEN + Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_RESERVED_LEN) / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_NUM_INDICES_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			else if( null == (track.indices = new JFLAC__StreamMetadata_CueSheet_Index[track.num_indices]) ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_MEMORY_ALLOCATION_ERROR;
			}

			for( int i = 0; i < track.num_indices; i++ ) {
				//FLAC__ASSERT(FLAC__STREAM_METADATA_CUESHEET_INDEX_OFFSET_LEN % 8 == 0);
				final JFLAC__StreamMetadata_CueSheet_Index ti = new JFLAC__StreamMetadata_CueSheet_Index();
				track.indices[i] = ti;
				len = Jformat.FLAC__STREAM_METADATA_CUESHEET_INDEX_OFFSET_LEN / 8;
				if( handle.read( buffer, 0, len ) != len ) {
					return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
				}
				...
				len = Jformat.FLAC__STREAM_METADATA_CUESHEET_INDEX_NUMBER_LEN / 8;
				if( handle.read( buffer, 0, len ) != len ) {
					return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
				}
				...
				len = Jformat.FLAC__STREAM_METADATA_CUESHEET_INDEX_RESERVED_LEN / 8;
				if( handle.read( buffer, 0, len ) != len ) {
					return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
				}
			}
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_cuesheet_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_CueSheet block)
	{
		...
		try {
			int len = Jformat.FLAC__STREAM_METADATA_CUESHEET_MEDIA_CATALOG_NUMBER_LEN / 8;
			if( handle.read( block.media_catalog_number, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}

			//FLAC__ASSERT(FLAC__STREAM_METADATA_CUESHEET_LEAD_IN_LEN % 8 == 0);
			len = Jformat.FLAC__STREAM_METADATA_CUESHEET_LEAD_IN_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = (Jformat.FLAC__STREAM_METADATA_CUESHEET_IS_CD_LEN + Jformat.FLAC__STREAM_METADATA_CUESHEET_RESERVED_LEN) / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_CUESHEET_NUM_TRACKS_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			else if( null == (block.tracks = new JFLAC__StreamMetadata_CueSheet_Track[block.num_tracks]) ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_MEMORY_ALLOCATION_ERROR;
			}

			for( int i = 0; i < block.num_tracks; i++ ) {
				block.tracks[i] = new JFLAC__StreamMetadata_CueSheet_Track();
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_picture_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_Picture block)
	{
		//FLAC__Metadata_SimpleIteratorStatus status;
		...
		try {
			...
			int len = Jformat.FLAC__STREAM_METADATA_PICTURE_TYPE_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			byte[] data;
			if( null == (data = read_metadata_block_data_picture_cstring_cb_( handle, Jformat.FLAC__STREAM_METADATA_PICTURE_MIME_TYPE_LENGTH_LEN )) ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;// status;
			}
			block.mime_type = new String( data, JFLAC__StreamMetadata_Picture.MIME_ENCODING );

			if( null == (data = read_metadata_block_data_picture_cstring_cb_( handle, Jformat.FLAC__STREAM_METADATA_PICTURE_DESCRIPTION_LENGTH_LEN ) ) ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;// status;
			}
			block.description = new String( data, JFLAC__StreamMetadata_Picture.DESCRIPTION_ENCODING );

			//FLAC__ASSERT(FLAC__STREAM_METADATA_PICTURE_WIDTH_LEN % 8 == 0);
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_WIDTH_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_HEIGHT_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_DEPTH_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_COLORS_LEN / 8;
			if( handle.read( buffer, 0, len ) != len ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;
			}
			...
			if( null == (block.data = read_metadata_block_data_picture_cstring_cb_( handle, Jformat.FLAC__STREAM_METADATA_PICTURE_DATA_LENGTH_LEN )) ) {
				return FLAC__METADATA_SIMPLE_ITERATOR_STATUS_READ_ERROR;//status;
			}
	...
	private static int /* FLAC__Metadata_SimpleIteratorStatus */ read_metadata_block_data_unknown_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Read read_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_Unknown block, final int block_length)
	{
		if( block_length == 0 ) {
			block.data = null;
		}
			...
			final JFLAC__StreamMetadata block)
	{
		switch( block.type ) {
			case Jformat.FLAC__METADATA_TYPE_STREAMINFO:
				return read_metadata_block_data_streaminfo_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_StreamInfo)block/*.data.stream_info*/ );
			case Jformat.FLAC__METADATA_TYPE_PADDING:
				return read_metadata_block_data_padding_cb_( handle, /*handle, seek_cb,*/ (JFLAC__StreamMetadata_Padding)block/*.data.padding*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_APPLICATION:
				return read_metadata_block_data_application_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_Application)block/*.data.application*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_SEEKTABLE:
				return read_metadata_block_data_seektable_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_SeekTable)block/*.data.seek_table*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_VORBIS_COMMENT:
				return read_metadata_block_data_vorbis_comment_cb_( handle, /*handle, read_cb, seek_cb,*/ (JFLAC__StreamMetadata_VorbisComment)block/*.data.vorbis_comment*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_CUESHEET:
				return read_metadata_block_data_cuesheet_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_CueSheet)block/*.data.cue_sheet*/ );
			case Jformat.FLAC__METADATA_TYPE_PICTURE:
				return read_metadata_block_data_picture_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_Picture)block/*.data.picture*/ );
			default:
				return read_metadata_block_data_unknown_cb_( handle, /*handle, read_cb,*/ (JFLAC__StreamMetadata_Unknown)block/*.data.unknown*/, block.length );
		}
	}
	...
	static boolean write_metadata_block_header_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Write write_cb,
			...
			final JFLAC__StreamMetadata block)
	{
		final byte buffer[] = new byte[Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH];

		//FLAC__ASSERT(block.length < (1u << FLAC__STREAM_METADATA_LENGTH_LEN));
		/* double protection */
		if( block.length >= (1 << Jformat.FLAC__STREAM_METADATA_LENGTH_LEN) ) {
			return false;
		}
		...
		try {
			handle.write( buffer, 0, Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH );
		} catch(final IOException e) {
			return false;
		}
	...
	static int write_metadata_block_header_(final RandomAccessFile file, final JFLAC__StreamMetadata block)
	{
		//FLAC__ASSERT(0 != file);
	...
	private static boolean write_metadata_block_data_streaminfo_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Write write_cb,
			...
			final JFLAC__StreamMetadata_StreamInfo block)
	{
		final byte buffer[] = new byte[Jformat.FLAC__STREAM_METADATA_STREAMINFO_LENGTH];
		...
		try {
			handle.write( buffer, 0, Jformat.FLAC__STREAM_METADATA_STREAMINFO_LENGTH );
		} catch(final IOException e) {
			return false;
		}
	...
	private static boolean write_metadata_block_data_seektable_cb_(//FLAC__IOHandle handle, FLAC__IOCallback_Write write_cb,
			final RandomAccessFile handle,// java: changed
			final JFLAC__StreamMetadata_SeekTable block)
	{
		final byte buffer[] = new byte[Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH];
		...
		try {
			for( int i = 0; i < block.num_points; i++ ) {
				...
				handle.write( buffer, 0, Jformat.FLAC__STREAM_METADATA_SEEKPOINT_LENGTH );
			}
		} catch(final IOException e) {
			return false;
		}
			...
			final JFLAC__StreamMetadata_VorbisComment block)
	{
		...
		try {
			byte string[] = block.vendor_string.getBytes( JFLAC__StreamMetadata_VorbisComment.ENCODING );
			...
			for( int i = 0; i < block.num_comments; i++ ) {
				string = block.comments[i].getBytes( JFLAC__StreamMetadata_VorbisComment.ENCODING );
				pack_uint32_little_endian_( string.length, buffer, entry_length_len );
				handle.write( buffer, 0, entry_length_len );
				handle.write( string, 0, string.length );
			}
			...
			final JFLAC__StreamMetadata_CueSheet block)
	{
		...
		try {
			...
			for( int i = 0; i < block.num_tracks; i++ ) {
				final JFLAC__StreamMetadata_CueSheet_Track track = block.tracks[i];

				//FLAC__ASSERT(FLAC__STREAM_METADATA_CUESHEET_TRACK_OFFSET_LEN % 8 == 0);
				len = Jformat.FLAC__STREAM_METADATA_CUESHEET_TRACK_OFFSET_LEN / 8;
			...
			final JFLAC__StreamMetadata_Picture block)
	{
		...
		try {
			...
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_MIME_TYPE_LENGTH_LEN / 8;
			int slen = block.mime_type.length();// java: using US_ASCII, so byte length = string length
			pack_uint32_( slen, buffer, 0, len );
			handle.write( buffer, 0, len );
			handle.write( block.mime_type.getBytes( JFLAC__StreamMetadata_Picture.MIME_ENCODING ), 0, slen );

			len = Jformat.FLAC__STREAM_METADATA_PICTURE_DESCRIPTION_LENGTH_LEN / 8;
			final byte[] description = block.description.getBytes( JFLAC__StreamMetadata_Picture.DESCRIPTION_ENCODING );
			...
			len = Jformat.FLAC__STREAM_METADATA_PICTURE_DATA_LENGTH_LEN / 8;
			...
			final JFLAC__StreamMetadata block)
	{
		...
		switch( block.type ) {
			case Jformat.FLAC__METADATA_TYPE_STREAMINFO:
				return write_metadata_block_data_streaminfo_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_StreamInfo)block/*.data.stream_info*/ );
			case Jformat.FLAC__METADATA_TYPE_PADDING:
				return write_metadata_block_data_padding_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_Padding)block/*.data.padding*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_APPLICATION:
				return write_metadata_block_data_application_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_Application)block/*.data.application*/, block.length );
			case Jformat.FLAC__METADATA_TYPE_SEEKTABLE:
				return write_metadata_block_data_seektable_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_SeekTable)block/*.data.seek_table*/ );
			case Jformat.FLAC__METADATA_TYPE_VORBIS_COMMENT:
				return write_metadata_block_data_vorbis_comment_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_VorbisComment)block/*.data.vorbis_comment*/ );
			case Jformat.FLAC__METADATA_TYPE_CUESHEET:
				return write_metadata_block_data_cuesheet_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_CueSheet)block/*.data.cue_sheet*/ );
			case Jformat.FLAC__METADATA_TYPE_PICTURE:
				return write_metadata_block_data_picture_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_Picture)block/*.data.picture*/ );
			default:
				return write_metadata_block_data_unknown_cb_( handle, /*handle, write_cb,*/ (JFLAC__StreamMetadata_Unknown)block/*.data.unknown*/, block.length );
		}
	}

	/** @return FLAC__Metadata_SimpleIteratorStatus */
	static int write_metadata_block_data_(final RandomAccessFile file, final JFLAC__StreamMetadata block)
	{
		//FLAC__ASSERT(0 != file);
		//FLAC__ASSERT(0 != status);
