...
final class JFLAC__OggDecoderAspect extends Jogg_mapping {
	...
	private final Jogg_sync_state sync_state = new Jogg_sync_state();
	...
	private final Jogg_page working_page = new Jogg_page();
	/** only if true will the following vars be valid */
	...
	private final Jogg_packet working_packet = new Jogg_packet();
	...
	final boolean FLAC__ogg_decoder_aspect_init()
	{
	...
	final void FLAC__ogg_decoder_aspect_finish()
	{
		this.sync_state.ogg_sync_clear();
		this.stream_state.ogg_stream_clear();
	}

	final void FLAC__ogg_decoder_aspect_set_serial_number(final int value)
	{
		this.use_first_serial_number = false;
		this.serial_number = value;
	}

	final void FLAC__ogg_decoder_aspect_set_defaults()
	{
		this.use_first_serial_number = true;
	}

	final void FLAC__ogg_decoder_aspect_flush()
	{
		this.stream_state.ogg_stream_reset();
		this.sync_state.ogg_sync_reset();
		this.end_of_stream = false;
		this.have_working_page = false;
	}

	final void FLAC__ogg_decoder_aspect_reset()
	{
     ...
     */
	final int /*FLAC__OggDecoderAspectReadStatus*/ FLAC__ogg_decoder_aspect_read_callback_wrapper(final byte buffer[], int bytes,
			final JFLAC__OggDecoderAspectReadCallbackProxy read_callback, final JFLAC__StreamDecoder decoder/*, final Object client_data*/)
			throws IOException
	{
		 ...
		 */
		bytes = 0;
		final Jogg_packet ogg_packet = this.working_packet;// java
		while( bytes < bytes_requested && ! this.end_of_stream ) {
			if( this.have_working_page ) {
				if( this.have_working_packet ) {
					...
					final int ret = this.stream_state.ogg_stream_packetout( ogg_packet );
					if( ret > 0 ) {
				...
				final int ret = this.sync_state.ogg_sync_pageout( this.working_page );
				if( ret > 0 ) {
					/* got a page, grab the serial number if necessary */
					if( this.need_serial_number ) {
						this.stream_state.serialno = this.serial_number = this.working_page.ogg_page_serialno();
						this.need_serial_number = false;
					}
					if( this.stream_state.ogg_stream_pagein( this.working_page ) == 0 ) {
					...
					final int oggbuf_offset = this.sync_state.ogg_sync_buffer( ogg_bytes_to_read );
					...
					if( oggbuf_offset < 0 ) {
						...
						*/
						if( this.sync_state.ogg_sync_wrote( ogg_bytes_to_read ) < 0 ) {
