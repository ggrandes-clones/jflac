	...
	JFLAC__StreamDecoderErrorCallback
{
	...
	private final long chain_prepare_for_write_(final boolean use_padding)
	{
		...
		/* check sizes of all metadata blocks; reduce padding size if necessary */
		{
			for( JFLAC__Metadata_Node node = this.head; node != null; node = node.next ) {
				if( node.data.length >= (1 << Jformat.FLAC__STREAM_METADATA_LENGTH_LEN) ) {
					if( node.data.type == Jformat.FLAC__METADATA_TYPE_PADDING ) {
						node.data.length = (1 << Jformat.FLAC__STREAM_METADATA_LENGTH_LEN) - 1;
						current_length = chain_calculate_length_();
					} else {
	...
	public final boolean FLAC__metadata_chain_check_if_tempfile_needed(final boolean use_padding)
	{
		...
		if( use_padding ) {
			final JFLAC__Metadata_Node node = this.tail;
			/* if the metadata shrank and the last block is padding, we just extend the last padding block */
			if( current_length < this.initial_length && node.data.type == Jformat.FLAC__METADATA_TYPE_PADDING ) {
				lbs_state = LBS_SIZE_CHANGED;
				lbs_size = node.data.length + (this.initial_length - current_length);
			}
			/* if the metadata shrank more than 4 bytes then there's room to add another padding block */
			else if( current_length + (long)Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH <= this.initial_length ) {
				lbs_state = LBS_BLOCK_ADDED;
				lbs_size = this.initial_length - (current_length + (long)Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH);
			}
			/* if the metadata grew but the last block is padding, try cutting the padding to restore the original length so we don't have to rewrite the whole file */
			else if( current_length > this.initial_length ) {
				final long delta = current_length - this.initial_length;
				if( node.data.type == Jformat.FLAC__METADATA_TYPE_PADDING ) {
					/* if the delta is exactly the size of the last padding block, remove the padding block */
					if( (long)node.data.length + (long)Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH == delta ) {
		...
		/* check sizes of all metadata blocks; reduce padding size if necessary */
		{
			for( JFLAC__Metadata_Node node = this.head; node != null ; node = node.next ) {
				...
				if( block_len >= (1 << Jformat.FLAC__STREAM_METADATA_LENGTH_LEN) ) {
					if( node.data.type == Jformat.FLAC__METADATA_TYPE_PADDING ) {
						block_len = (1 << Jformat.FLAC__STREAM_METADATA_LENGTH_LEN) - 1;
					} else {
						return false /* the return value doesn't matter */;
					}
				}
				current_length += (Jformat.FLAC__STREAM_METADATA_HEADER_LENGTH + block_len);
			}
